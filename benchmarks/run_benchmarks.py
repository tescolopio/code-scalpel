...
def run_benchmark(benchmark: Dict[str, Any], analyzer: CodeAnalyzer) -> Dict[str, Any]:
    ...
    for item in result.dead_code[:10]:  # Limit to first 10
        print(f"  - {item.code_type}: {item.name} (line {item.line_start}, {int(item.confidence*100)}% confidence)")
...
def run_all_benchmarks() -> Dict[str, Any]:
    ...
    print(f"\n{'='*60}")
    print(f"OVERALL RESULT: {'✓ ALL CRITERIA MET' if overall_pass else '✗ SOME CRITERIA NOT MET'}")
    print(f"{'='*60}")

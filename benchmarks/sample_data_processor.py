...
class DataValidator:
    ...
    def validate(self, row: DataRow) -> Tuple[bool, List[str]]:
        ...
        for schema_field in self.schema.fields:
            if schema_field.required and schema_field.name not in row:
        ...
        for schema_field in self.schema.fields:
            if schema_field.name in row:
                value = row[schema_field.name]
                ...
                if value is not None and not isinstance(value, field.data_type):
                    try:
                        ...
                        errors.append(
                            f"Invalid type for {field.name}: expected {field.data_type.__name__}"
                        )
    ...
    def unused_validation_method(self, row: DataRow) -> bool:
        ...
        if not row:
            return False
        for schema_field in self.schema.fields:
            if schema_field.name not in row:
                return False
        return True
    ...
    def aggregate(self, rows: Iterator[DataRow]) -> Dict[Tuple, DataRow]:
        ...
        for key, group_rows in groups.items():
            ...
            for name, (agg_type, source) in self._aggregations.items():
                values = [
                    r.get(source) for r in group_rows if r.get(source) is not None
                ]
...
def merge_rows(
    row1: DataRow, row2: DataRow, conflict_strategy: str = "prefer_first"
) -> DataRow:
    """Merge two data rows."""
    result = row1.copy()

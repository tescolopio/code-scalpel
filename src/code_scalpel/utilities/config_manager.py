...
class ConfigManager:
    ...
    def load_from_dict(
        self, data: dict[str, Any], source: ConfigSource = ConfigSource.DEFAULT
    ):
        """Load configuration from dictionary."""
        for key, value in data.items():
            self.set(key, value, source)

    def save_to_file(self, file_path: str, include_metadata: bool = False):
        """
        ...
        if include_metadata:
            ...
            data = {key: config.value for key, config in self.config.items()}
    ...
    def _validate_value(self, key: str, value: Any):
        ...
        try:
            ...
            jsonschema.validate(
                instance={"value": value}, schema={"properties": {"value": schema}}
            )

...
class PathExplorer:
            ...
            if isinstance(node, ast.If):
                for branch, condition in self._get_branches(node):
                    ...
                    if self._is_feasible(new_path):
                        ...
                        heapq.heappush(
                            paths,
                            (-score, len(paths), branch, new_path)
                        )
            else:
                for child in ast.iter_child_nodes(node):
                    heapq.heappush(
                        paths,
                        (0, len(paths), child, path_info)
                    )
            
            if self._is_complete_path(path_info):
                yield path_info

    def _explore_coverage_guided(self, tree: ast.AST) -> Generator[PathInfo, None, None]:
    ...
    def _calculate_complexity_score(self, path_info: PathInfo) -> float:
        ...
        total_complexity = sum(
            self._get_constraint_complexity(c)
            for c in path_info.constraints
        )
    ...
    def _extend_path(self, path_info: PathInfo, 
                    condition: Any) -> PathInfo:
    ...
    def _should_continue(self) -> bool:
        ...
        if self.config.max_paths and len(self.exploration_history) >= self.config.max_paths:
            return False
    ...
    def _should_prune(self, path_info: PathInfo) -> bool:
        ...
        if not self._is_feasible(path_info):
            return True
